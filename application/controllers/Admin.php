...
class Admin extends CI_Controller
{
  ...
  public function addKaryawan()
  {
    ...
    $config['cachedir']     = './assets/images/'; //string, the default is application/cache/
    $config['errorlog']     = './assets/images/'; //string, the default is application/logs/
  ...
  public function delKaryawan($id)
  {
    ...
    $qr = $poto['qr'];
    unlink('./assets/images/qr/' . $qr);
    $old_poto = $poto['poto'];
    if ($old_poto != 'default.png') {
      unlink('./assets/images/user/' . $old_poto);
    }
  ...
  public function edpoto($id)
  {
    ...
    if ($upload_photo) {
      ...
      if ($this->upload->do_upload('poto')) {
        if ($old_poto != 'default.png') {
          unlink(FCPATH . 'assets/images/user/' . $old_poto);
        }
  ...
  public function edProfil()
  {
    ...
    $userPass = $this->db->get_where('user', ['id' => $id])->row_array();
    if (password_verify($pass, $userPass['password'])) {
      ...
      $user = $this->db->get_where('user', ['email' => $email])->row_array();
      if ($user) {
        $this->db->set(['nama' => $nama, 'perusahaan' => $usaha])->where('id', $id)->update('user');
        $this->session->set_flashdata('berhasil', 'Update Data');
        redirect('admin/profile');
      } else {
        $this->db->set(['nama' => $nama, 'email' => $email, 'perusahaan' => $usaha])->where('id', $id)->update('user');
  ...
  {
    $poto = $this->db->get_where('user', ['id' => $id])->row_array();
    $old_poto = $poto['poto'];
    $upload_photo = $_FILES['poto']['name'];
    if ($upload_photo) {
      $config['allowed_types'] = 'gif|jpg|png|jpeg';
    ...
    if ($upload_photo) {
      ...
      if ($this->upload->do_upload('poto')) {
        if ($old_poto != 'logo.png') {
          unlink(FCPATH . 'assets/images/logo/' . $old_poto);
        }
        $new_image = $this->upload->data('file_name');
  ...
  public function downloadBarcode($id)
  {
    $user = $this->db->get_where('karyawan', ['id' => $id])->row_array();
    if ($user) {
      redirect('admin/barcode/?name=ID&size=60&print=true&text=' . $user['nik']);
    }
  }
